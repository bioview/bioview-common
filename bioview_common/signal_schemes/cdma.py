...
class CdmaRxProcessor(RxProcessor):
    def __init__(self, scheme: "CdmaScheme", tx_idx: int, if_freq: float, if_filter_bw: float):
        self.scheme = scheme
        self.tx_idx = tx_idx
    ...
    def process_chunk(self, rx_samples: np.ndarray) -> np.ndarray:
        ...
        # 1. De-spread by multiplying with the exact Tx code
        tx_code = self.scheme.get_code_at(self.tx_idx, len(rx_samples), self.accumulated_sample_idx)
        despread_samples = rx_samples * tx_code
        
        # 2. Bandpass filter the despread signal
        filt_data, new_filter_state = apply_filter(
            despread_samples, self.filt, zi=self.filter_state
        )
        self.filter_state = new_filter_state

        # 3. Downconvert to baseband
...
class CdmaScheme(BurstEnvelopeMixin, SignalScheme):
    ...
    def __init__(
        ...
        # For CDMA, typically all channels share the same IF. We'll use the first IF provided.
        # But if the user provides multiple, we use them (maybe they want hybrid CDMA/FDMA).
    ...
    def _generate_codes(self):
        ...
        # Walsh-Hadamard requires length to be a power of 2
        ...
        if self.code_type == "Walsh-Hadamard":
            ...
            # Fallback PRBS (Pseudo-Random Binary Sequence)
    ...
    def create_rx_processor(
        ...
        return CdmaRxProcessor(self, tx_idx, self.if_freq[tx_idx] if tx_idx < len(self.if_freq) else 100e3, if_filter_bw)
